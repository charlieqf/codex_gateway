...
describe("Research Worker controlled-beta workflow", () => {
  ...
  it.each([
    ...
    const postSafetyTopicBoundaryParagraph = `${[
      Array.from(
        { length: 7 },
        () =>
          "研究设计与人群差异部分仅综合所引公开摘要证据，比较研究设计与方法差异，并明确证据边界和适用限制"
      ).join("。"),
      "公开摘要证据支持研究比较的方法与解释边界"
    ].join("。")}。[1]`;
    expect(
      postSafetyTopicBoundaryParagraph.match(/\p{Script=Han}/gu)
        ?.length ?? 0
    ).toBe(335);
    const postSafetyNearMinimumBodyFragment = (() => {
      const [, ...remaining] = skillBodyFragment(20).split(
        /\n\n(?=##\s)/u
      );
      ...
      return [
        `## 研究设计与人群差异\n\n${postSafetyTopicBoundaryParagraph}\n\n${Array.from(
          { length: 4 },
          () => unsafeSectionPaddingSentence
        ).join("。")}。[1]`,
        ...remaining.map(
        ...
        async generate(modelInput) {
          ...
          if (
            ...
            return {
              ...
              gatewayRequestId:
                "req_sharded_multi_skill_peer_contract_failure",
              usage: {
                promptTokens: 100,
                completionTokens: 100,
                totalTokens: 200
          ...
          if (
            ...
            return {
              ...
              gatewayRequestId:
                "req_sharded_multi_skill_peer_contract_retry",
              usage: {
                promptTokens: 100,
                completionTokens: 100,
                totalTokens: 200
      ...
      expect(validationEvents).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
            stage: "validate_outputs",
            attempt: 6,
            errorCodes: ["peer_review_contract_error"]
          })
        ])
      );
    }
