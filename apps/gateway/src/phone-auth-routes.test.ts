...
describe("internal phone auth v1 routes", () => {
  it("takes a newly provisioned free account through the existing Desktop login and resolver contract", async () => {
    ...
    try {
      ...
      expect(current.json().credential.token.tokensPerDay).toBeNull();
      expect(current.json().credential.token.tokensTotal).toBe(1_000_000);
      expect(fixture.store.listEntitlements({ subjectId: created.json().subject.id })[0]?.policySnapshot.tokensTotal).toBe(1_000_000);
