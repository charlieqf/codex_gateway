...
describe("internal phone auth v1 routes", () => {
  it("takes a newly provisioned free account through the existing Desktop login and resolver contract", async () => {
    ...
    try {
      ...
      expect(current.json().credential.token.tokensPerDay).toBe(100_000);
      expect(fixture.store.listEntitlements({ subjectId: created.json().subject.id })[0]?.policySnapshot.tokensPerDay).toBe(100_000);
