...
import { InMemoryRequestRateLimiter, type RateLimitPermit } from "./rate-limiter.js";
...
describe("InMemoryRequestRateLimiter", () => {
  it("limits requests per minute and returns retry_after_seconds", () => {
    ...
    const limiter = new InMemoryRequestRateLimiter({ now: () => now });

    const first = permit(limiter.acquire({ scope: "credential", key: "cred_1", policy }));
    first.release();

    const limited = limiter.acquire({ scope: "credential", key: "cred_1", policy });
    ...
    permit(limiter.acquire({ scope: "credential", key: "cred_1", policy })).release();
  });
  ...
  it("limits requests per UTC day", () => {
    ...
    permit(limiter.acquire({ scope: "credential", key: "cred_1", policy: dailyPolicy })).release();
    const limited = limiter.acquire({ scope: "credential", key: "cred_1", policy: dailyPolicy });
  ...
  it("resets selected request windows without clearing active concurrency", () => {
    ...
    const first = permit(limiter.acquire({ scope: "credential", key: "cred_1", policy: limitedPolicy }));
    const concurrentLimited = limiter.acquire({ scope: "credential", key: "cred_1", policy: limitedPolicy });
    expect(rejection(concurrentLimited).limitKind).toBe("concurrency");
    first.release();

    const dayLimited = limiter.acquire({ scope: "credential", key: "cred_1", policy: limitedPolicy });
    ...
    permit(limiter.acquire({ scope: "credential", key: "cred_1", policy: limitedPolicy })).release();

    now = new Date("2026-01-02T00:01:00Z");
    const active = permit(limiter.acquire({ scope: "credential", key: "cred_1", policy: limitedPolicy }));
  ...
  it("limits concurrent requests until a permit is released", () => {
    ...
    const first = permit(limiter.acquire({ scope: "credential", key: "cred_1", policy: concurrencyPolicy }));
    const limited = limiter.acquire({ scope: "credential", key: "cred_1", policy: concurrencyPolicy });
    ...
    first.release();
    permit(limiter.acquire({ scope: "credential", key: "cred_1", policy: concurrencyPolicy })).release();
  });
  ...
  it("prunes released state after minute and day windows expire", () => {
    ...
    const first = permit(limiter.acquire({ scope: "credential", key: "cred_1", policy: generousPolicy }));
    expect(stateCount(limiter)).toBe(1);
  ...
  it("prunes idle per-minute keys when the minute changes", () => {
    ...
    permit(
      limiter.acquire({ scope: "credential", key: "phone-auth:phone:first", policy: minuteOnlyPolicy })
    ).release();
    expect(stateCount(limiter)).toBe(1);

    now = new Date("2026-01-01T00:01:00Z");
    permit(
      limiter.acquire({ scope: "credential", key: "phone-auth:phone:second", policy: minuteOnlyPolicy })
    ).release();
...
function stateCount(limiter: InMemoryRequestRateLimiter): number {
