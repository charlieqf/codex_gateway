...
describe("gateway phase 1 routes", () => {
    ...
    try {
      await withTemporaryEnv(
        ...
        async () => {
          ...
          const sessionId = sessionIdForGoldencodeMember("goldencode-tiankuan");
  ...
  it("flushes response.created before GoldenCode finishes and does not retry auto tools", async () => {
    ...
    const appEnv = {
      ...
      MEDCODE_TENCENT_TOKENHUB_API_KEY: "provider-test-key",
      MEDCODE_TENCENT_TOKENHUB_BASE_URL: server.baseUrl,
      MEDCODE_TIANKUAN_API_KEY: "provider-test-key",
      MEDCODE_TIANKUAN_BASE_URL: server.baseUrl,
  ...
  it("pauses the current subscription through a client credential", async () => {
    const { store, headers } = createCredentialBackedStore();
    store.createPlan({
      id: "plan_client_pause_v1",
      displayName: "Client Pause Plan",
      scopeAllowlist: ["code"],
      policy: unrestrictedTokenPolicy(),
      now: new Date("2026-01-01T00:00:00Z")
    });
    ...
    const paused = await app.inject({
      ...
      headers,
      payload: { reason: "client requested pause" }
    });
    expectRequestIdHeader(paused);
    ...
    expect(paused.json()).toMatchObject({
      ...
      plan: {
        display_name: "Client Pause Plan",
        scope_allowlist: ["code"]
      },
      entitlement: {
        period_kind: "unlimited",
    ...
    expect(current.json().credential).not.toHaveProperty("token");
  ...
  it("rejects client subscription pause without an active entitlement", async () => {
    ...
    expect(missing.json().error.code).toBe("entitlement_not_found");
    ...
    expect(crossSubject.statusCode).toBe(400);
    expect(crossSubject.json().error.code).toBe("invalid_request");
...
async function startGoldencodeRuntimeServers(): Promise<{
  ...
  const tiankuan = await startOpenAICompatibleCaptureServer(
    "tiankuan-goldencode-ok",
    captured.tiankuan,
    { "x-request-id": "tiankuan_req_goldencode" }
  );
  ...
  return {
    ...
    env: {
      ...
      MEDCODE_TIANKUAN_API_KEY: "provider-test-key",
      MEDCODE_TIANKUAN_BASE_URL: tiankuan.baseUrl,
      MEDCODE_TOKENSWITCH_API_KEY: "provider-test-key",
