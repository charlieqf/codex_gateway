...
RUN npm run build
RUN node node_modules/vitest/vitest.mjs run packages/store-sqlite/src/free-paid-quota.test.ts packages/store-sqlite/src/quota-dashboard.test.ts
...
RUN npx vitest run apps/admin-cli/src/index.test.ts -t "checks and restores an existing MedEvidence access bundle"

FROM ${BASE_GATEWAY_IMAGE} AS runtime
ARG GATEWAY_REVISION
USER root
COPY --from=verify /app/apps/gateway /app/apps/gateway
COPY --from=verify /app/apps/admin-cli /app/apps/admin-cli
...
RUN chmod -R a=rX /app/apps/gateway /app/apps/admin-cli /app/packages/core /app/packages/store-sqlite
USER codexgw
