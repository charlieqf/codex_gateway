...
| `upstream_account` | 服务端持有的上游 AI 服务登录态或账号记录，包含 provider、登录态引用、健康状态和调度状态。 |
...
| `session` | 用户与 AI 的连续交互上下文。MVP 下归属一个 subject，并绑定唯一 upstream account。 |
...
3. 检查过期、吊销、scope、限流。
4. 为新 session 选择 upstream account。
5. 为已有 session 使用固定 upstream account。
...
| `id` | 网关会话 ID。 |
| `subject_id` | 会话归属。 |
| `upstream_account_id` | 首次创建时绑定的上游账号。 |
...
2. `POST /sessions` 创建新 session，并绑定当前 scheduler 选择的 upstream account。
3. `POST /sessions/{id}/messages` 必须校验 session 属于当前 subject。
4. v2 多上游账号下，已有 session 始终回到创建时绑定的 upstream account。
5. 如果绑定的 upstream account 不可用，默认返回结构化错误，不自动迁移到其他 upstream account。

### 4.5 Upstream Account Store
...
MVP 只有一条 active upstream account。v2 支持多条。
...
interface ProviderAdapter {
  kind: string
  health(sub: UpstreamAccount): Promise<ProviderHealth>
  refresh(sub: UpstreamAccount): Promise<RefreshResult>
...
1. 每 upstream account 并发数。
2. 每 upstream account provider 错误率冷却。
3. 全局队列或公平调度。
...
```text
SingleUpstreamAccountScheduler:
  if session has upstream_account_id:
    return that upstream account
  return only active upstream account
```
...
PoolScheduler:
  if session has upstream_account_id:
    return that upstream account if usable
    otherwise return subscription_unavailable

  candidates = active upstream accounts excluding cooldown
  choose by least_recently_used / least_inflight / health
  return selected upstream account
```

S8 “停用其中一个上游账号，其他账号承接负载”解释为：新 session 和未绑定请求在 10 秒内不再选择被停用上游账号。已有 session 因需求明确“不做跨上游账号迁移”，默认不自动迁移。
...
网关创建 session，绑定 subject 和 upstream account。
...
3. 校验 scope 能力。
4. 校验限流。
5. 使用 session 绑定的 upstream account。
...
| `subscription_unavailable` | 503 | 绑定上游账号不可用；错误码保留旧名用于兼容。 |
...
2. 单 upstream account。
3. 多 access credential。
4. `subject`、`credential`、`session`、`upstream_account` 四类核心状态。
...
1. `upstream_account_id` 字段。
2. Session 创建时绑定 upstream account。
3. Scheduler 接口。
4. UpstreamAccount health 和 cooldown 字段。
5. 上游账号维度观察事件。
6. Provider adapter 抽象。

v2 新增能力：

1. 多 upstream account 接入。
2. 新 session 在健康上游账号间调度。
3. 某 upstream account 被禁用后，新流量在 10 秒内不再命中。
4. 单凭据限流不影响其他凭据。
5. 上游账号维度健康、错误率、延迟可查。
...
1. 已有 session 跨 upstream account 自动迁移。
2. 多网关实例强一致。
3. 复杂优先级队列。
...
gateway:
  ...
  use session.upstream_account_id
...
adapter:
  ...
  marks upstream account reauth_required

gateway:
  new requests return provider_reauth_required
  observation records affected upstream account
```
...
| FR-4 上游账号凭据托管 | Upstream Account Store + Provider Credential Ref + Adapter Refresh |
...
| FR-6 观察与审计 | Observation Writer + 凭据/上游账号维度统计 |
| NFR-1 性能 | Streaming Proxy + first_byte_ms 观测 + 压测 |
| NFR-2 可用性 | MVP 单实例恢复流程；v2+ 共享状态和重连 |
| NFR-3 可扩展性 | MVP 预留 upstream account/session/scheduler 字段 |
...
6. v2 中已有 session 的绑定 upstream account 故障时，是立即报错、允许手工迁移，还是提供只读历史。
