...
新增后台 subjects/resolve，先按稳定外部 ID 或唯一手机号关联账户。真正的新用户再调用原 POST /subjects，一个事务完成账户、Key、免费权益和手机号身份，姓名可空；旧账户保留原 Subject、姓名、Key 和权益。

本地类型检查和 338 项测试通过。公网地址仍为 https://goldencode.instmarket.com.au:1443，具体上线状态以本轮发布验收记录为准。

客户端需按 SMS→v1 重新验证，并移除 v2 授权及支付前必须已有模型 Key／subjectId 的前提。原客户端 65 项测试不代表本轮真实联调完成。身份后端需接入 resolve，产品仍需提供正式协议链接，真实短信和支付验收由相关团队共同完成。
