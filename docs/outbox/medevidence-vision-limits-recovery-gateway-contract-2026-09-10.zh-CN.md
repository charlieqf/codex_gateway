...
Gateway 已于北京时间 17:59 部署 R760，运行提交 `8dab89d`。
验证证据见[运维发布记录](../operations/r760-vision-limits-recovery-release-2026-09-10.zh-CN.md)；
客户端源码和安装包由客户端团队维护。
...
客户端团队随后报告：统一 8 张图片预算、保留原图与 view_images 重读、历史图片不再阻止
新增上传、图片错误专用恢复已实施，203 项测试与两包类型检查通过，尚未发布 EXE。
Gateway 团队只读检查其更新后的同日故障文档和源码，没有修改客户端文件，也没有代跑该 203 项测试。

**打包前仍需补齐两个协议接入点：**

1. `packages/opencode/src/provider/error.ts` 的 `imageLimit()` 目前只识别旧英文错误文案，
   以及拟议的顶层 `image_count_exceeded`、`image_too_large`。本批上线合同保留顶层
   `invalid_request`，使用数字 `error.image_limit_contract_version=1` 和
   `error.image_limit.kind`。当前客户端没有解析这些字段，未命中图片分支后又把所有
   HTTP 413 归为 `context_overflow`，因此新版限制响应仍会误入全文压缩。
   请按本合同增加结构化分类，并用 12 张、单图超限、内联合计超限、request_bytes/actual=null
   的实际响应样本补测；保留对旧文案的兼容即可，不应以翻译文本作为主判断。
2. 当前客户端源码尚无 `x-medcode-vision-recovery-contract` 请求头接入。
   Gateway 的有限恢复已经上线，但这些请求仍走未启用行为。完成 JSON、Chat SSE、Responses
   SSE 及工具纠错终态的严格停止合同验收后，在实际视觉请求处发送值 `1`；覆盖所有模型请求
   发送路径，禁止 SDK／工具／会话在明确停止后再自动发起新请求。

客户端已有通用严格停止字段解析，不等于本批 Gateway 线上响应样本与全部工具恢复分支
已经联合验收。客户端文档中“视觉有限重试仍需 Gateway 实施”的状态也应由其团队更新为
“Gateway 已上线，待客户端声明能力并联合验收”。本次不要求 Gateway 再等待客户端 EXE 才部署；
显式能力请求头确保现有安装包不会自动开启新策略。
