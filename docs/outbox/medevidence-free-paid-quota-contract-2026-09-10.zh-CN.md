...
> **状态：本文的免费额度条款已被取代（2026-09-11）。** 免费额度已从"每日重置"改为
> 注册一次性 1,000,000 token（`plan_free_once_1m_v1`），付费模板数值也已调整。现行有效版本见
> [一次性免费额度合同 v2](./medevidence-free-once-quota-contract-2026-09-11.zh-CN.md)。本文的
> 双账本记账、取消与重置冲突条款仍然有效；阅读时以 v2 为准。
