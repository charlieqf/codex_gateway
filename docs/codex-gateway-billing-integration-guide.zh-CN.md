...
- 终端用户在收费团队站点注册时，收费后端调用 MedEvidence `create_subject` API 拿到 `subject_id` 和 opaque key；用户或客户端只保存这把 opaque key，并通过当前 MedEvidence 客户端接入方式使用模型能力。
...
Billing 路由有独立后台调用限流，当前测试环境默认 `120 rpm / 10000 rpd / 8 concurrent`，超限返回 `429 rate_limited` 并带 `Retry-After`。请收费团队启用指数退避，并支持 dead-letter queue。注册专项限流会在 subject 开户扩大到生产灰度或更高流量前单独确认。
...
- **key 只在响应里返回一次**：MedEvidence 不存储 opaque key 原文，只保存校验所需摘要和加密后的底层凭据。响应返回后，收费团队**必须立即展示给用户或安全留存**；若遗失只能通过 §4.4 轮换得到新 key。
...
当前测试环境 subject 开户路径先使用 §3.3 的 Billing Admin 统一后台限流。正式开放更大注册流量前，MedEvidence 可能叠加注册路径专项限流；目标策略如下，最终是否启用和数值以联调 handoff 为准：
