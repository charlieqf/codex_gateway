...
| MedEvidence Desktop access version gate | `GATEWAY_DESKTOP_VERSION_GATE`, `GATEWAY_MINIMUM_DESKTOP_VERSION` and `GATEWAY_DESKTOP_DOWNLOAD_URL`, read by Gateway at startup | Recreate Gateway only after the source supporting the selected mode is deployed; `medevidence_all` scopes the complete route set to MedEvidence while preserving shared Gateway clients | Keep the public download URL actionable and use the audited `restore-medevidence-access` control path for existing Subject/Phone identity/paused-entitlement recovery |
...
| GoldenCode native failover mode, subject rollout list and provider quota cooldown | Startup environment; [implementation and defaults](./goldencode-failover-implementation-2026-09-07.zh-CN.md); deployed in `b4fbed4` | Setting changes recreate Gateway only | P0 subject rollout and P1 whole-pool health are separate; P1 circuit state is process-local and resets on restart; disabled pool members remain disabled |
