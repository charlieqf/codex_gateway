...
> 修订注记（2026-09-11 稍后）：本文写作时基础 Free 为 `plan_free_daily_100k_v1`（100,000/日），
> 同日晚间已被一次性模型取代（`plan_free_once_1m_v1`，1,000,000 终身），见
> [免费额度一次性化发布记录](./r760-free-once-release-2026-09-11.zh-CN.md)。本文的付费模板数值仍然有效；
> 下文涉及"基础 Free 每日额度独立保留"的表述按一次性模型理解。
