...
| 方法 | 目的 |
| --- | --- |
| `health(upstreamAccount)` | 探测上游账号是否健康、是否需要重新授权 |
| `refresh(upstreamAccount)` | 尝试刷新 provider 登录态或明确返回 reauth |
...
| `subscription_unavailable` | 上游账号被禁用或 health 不可用；错误码保留旧名以兼容外部客户端 |
