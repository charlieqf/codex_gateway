...
- Max 保持现有 400k public context 和 128k output，不把接入 Pro / Standard 变成 Max 降级。
- Gateway 默认配置只启用 `medcode`；OpenRouter key 缺失、OpenRouter adapter 未启用或 Pro / Standard disabled 时，Gateway 仍能启动并服务 `medcode`。
- 旧 plan / entitlement / key 缺少 `feature_policy.medcode_models` 时，兼容规则是自动允许当前 Gateway 已启用的 MedCode public models（第一版为 `medcode` / `pro` / `standard`），不能把已发放 key 误判为无权限。
- request event / token budget / entitlement migration 必须是 additive；旧数据可读，旧 admin/report 命令不因新 nullable 字段失败。
- `/v1/models` 至少继续返回 `medcode` 及现有 limit metadata。新增 `pro` / `standard` 只在 Gateway registry enabled 后出现。
...
- Max / Pro / Standard 分别映射到哪个上游 runtime 和模型。
...
pro      -> OpenAICompatibleProviderAdapter(upstreamModel=z-ai/glm-5.2, reasoning disabled)
...
- strict client-defined tools 必须关闭 reasoning；第一版不实现 reasoning 文本与 JSON envelope 的剥离契约。
- adapter smoke / CI 必须断言 `message.reasoning` 不存在或为空，且 `reasoning_tokens` 为 0；如果 OpenRouter 后续拒绝 `effort:"none"` 或重新产生 reasoning tokens，应禁用对应 public model 并重新评估。
- 后续如要对 Pro / Standard 开启 reasoning，需要同时提高输出上限、验证成本和延迟，定义 reasoning/envelope 分隔和剥离规则，并确保 reasoning 内容不会进入普通客户端诊断。
...
不要把 `capabilities` 改成对象。新增模型权限应作为 `feature_policy` 的独立字段，并进入 plan 和 entitlement snapshot：
...
当前 `validateFeaturePolicy()` 会重建返回对象，只保留已解析字段；未知字段不会自然透传。新增 `medcode_models` 必须同步修改：
...
第一版兼容门禁必须把已发放 key 视为三档模型默认可用。也就是说，缺少
`medcode_models` 的旧 plan / entitlement / key 在 Gateway 已启用 `pro` / `standard`
后也应自动放行当前启用的 MedCode public models；只有显式 `medcode_models.allowed`
存在时，才对新增 public model 做限制。`medcode` 仍是历史默认模型豁免项，不能被
allow-list 拒绝。

第一版推荐同时做到：后端门禁在 `medcode_models` 缺失时 fail-open 到当前 enabled
public models，公开 serializer 输出有效 allow-list 时也包含 `medcode`，避免客户端 UI
误以为 Max 不可用。可以额外对现有 active entitlement 做一次 backfill，把
`medcode_models.allowed` 显式补为 `standard` / `pro` / `medcode`，但 backfill 不能成为唯一兼容机制。

建议默认映射：

| MedCode plan | `medcode_models.allowed` |
| --- | --- |
| Standard | `standard`, `pro`, `medcode` |
| Pro | `standard`, `pro`, `medcode` |
| Max | `standard`, `pro`, `medcode` |

兼容规则：

- `medcode` public model id 在第一版永远不被 `medcode_models` 门禁拒绝；它继续映射到现有 Codex / `gpt-5.5`。
- 旧 plan / entitlement 缺少 `medcode_models` 时，兼容期允许当前 Gateway 已启用的 MedCode public models。
...
- active entitlement 有 `medcode_models.allowed`，但请求了不允许的新增 public model（非 `medcode`）时，返回：

```json
{
  "error": {
    "code": "plan_capability_required",
    "message": "Current plan does not include this MedCode model."
  }
}
```
...
客户端可以据此禁用不可用档位，但不能把该接口当作某个模型的 live probe。

模型权限校验的实现落点要避免重复查询 entitlement。当前 `beginTokenBudget()` 内部会调用 `entitlementAccessForSubject()`，并在那里处理 active / expired / inactive / legacy 和 `forbidden_scope`。新增 `medcode_models` 后，推荐把 entitlement 解析抽成一个共享步骤：

```text
resolveChatEntitlementAccess(subject, credential, scope, publicModelId)
  -> active entitlement / plan status
  -> scope decision
  -> medcode_models decision
  -> effective token policy
```

然后：

- runtime dispatcher 在调用上游前使用同一个 resolved access 判断模型权限。
- token budget hook 接收 resolved access / effective token policy，不再重复查询 entitlement。
- `forbidden_scope`、`plan_expired`、`plan_inactive`、`plan_capability_required` 的行为矩阵集中在一个 helper 和一组单测里。

如果首个 PR 为了降低改动面选择把 `publicModelId` 传入 `beginTokenBudget()`，也必须保证 entitlement 只查询一次，且 scope/model/token policy 三类决策在同一处维护。

### Token Budget 和成本控制

Pro / Standard 第一版必须有实际拦截能力，而不仅是记录用量。

上线门槛：

- `maxPromptTokensPerRequest` 对 Pro / Standard 生效。
...
在这些门槛验证前，不应把 Pro / Standard 的 context window 开到 1M。
...
普通客户端诊断和 UI 只记录 public model id：`medcode`、`pro`、`standard`。不上传真实上游模型、OpenRouter provider routing、OpenRouter key 或供应商账号信息。
...
Pro / Standard 继续使用现有 Gateway strict client-defined tools：
...
这样避免不同上游的 native tool-call streaming delta、reasoning 混流、provider fallback 行为进入 Desktop。第一版 Pro / Standard 在 strict tools 路径必须关闭 reasoning，Gateway 只解析纯 JSON envelope；不支持“先 reasoning 文本、再 JSON envelope”的混合输出。
...
- `Pro` / `Standard` 第一版先标记 text-only。
- `Pro` / `Standard` 第一版先标记 `reasoning: false`，与 Gateway strict client-defined tools 的 JSON envelope 路线保持一致。
- 不在普通 UI 中显示 `gpt-5.5`、`GLM-5.2`、`DeepSeek V4 Pro`。
- 如果 `/gateway/credentials/current` 返回 allowed models，Desktop 可以禁用无权限模型。
...
public models: medcode / pro / standard
```
...
3. 在 MedCode provider 下展示三档模型：
   - `medcode` -> `Max`
   - `pro` -> `Pro`
   - `standard` -> `Standard`
4. 按本节配置设置 Desktop 侧能力声明：
   - `Max` `limit.context` 是 `400000`，保持现有行为。
   - `Pro` / `Standard` `limit.context` 是 `200000`。
   - `Max` output `128000`。
   - `Pro` / `Standard` 不单独设置 32k output 上限，按 Gateway `/v1/models` 返回的 `max_output_tokens` 配置；当前默认与 Max 持平为 `128000`。
   - 客户端需专门验证 `limit.output=128000`、`limit.context=200000` 时的 compaction / 截断预算计算，确保不会错误地把输出预留吞掉全部输入空间。
   - 第一版三档都设 `reasoning: false`。
   - `Pro` / `Standard` 第一版 text-only，不声明 PDF / file input。
5. Provider Test 只调用 `/gateway/credentials/current` 验证 key，不对每个模型发 live probe。
6. 如果 `/gateway/credentials/current` 返回 `feature_policy.medcode_models.allowed`，根据 allowed models 禁用无权限档位；如果该字段缺失，兼容期按 `/v1/models` 当前返回的 MedCode public models fail-open。
7. 模型调用错误按 Gateway 结构化错误处理：
   - `model_not_found`：模型未启用或客户端配置早于 Gateway。
   - `plan_capability_required`：当前套餐不含该模型。
   - `plan_expired` / `plan_inactive`：订阅不可用。
   - `rate_limited`：限流。
   - `upstream_timeout` / `upstream_unavailable`：上游暂时不可用。
8. client message / diagnostic event 只记录 public model id：`medcode`、`pro`、`standard`。不要记录或上传真实上游模型名、OpenRouter provider routing、OpenRouter key 或供应商账号信息。
9. E2E 覆盖：
   - 默认 `medcode/medcode` 仍可用。
   - 选择 `pro` / `standard` 后普通对话可用。
   - 选择 `pro` / `standard` 后医学问题仍能触发 `medevidence` 工具。
...
- 模型选择器如何在 `/gateway/credentials/current` 尚未返回 allowed models 时做兼容展示。
- 禁用无权限模型、展示 Gateway 结构化错误的 UI 文案。
...
- 旧客户端兼容 smoke：未启用 Pro / Standard 时，旧 key + `model=medcode` 的普通聊天、streaming、strict tools 仍走 Codex / `gpt-5.5`。
- `/v1/models` 只返回 enabled public models。
- `/v1/models` 返回现有兼容字段 `context_window`、`max_context_window`、`max_output_tokens`；Max 为 400k，Pro / Standard 为 200k public context。
...
- public model registry 为 `pro` / `standard` 构造独立 adapter/runtime 实例，不通过 `MessageInput` 动态传 model。
- 无权限模型返回 `plan_capability_required`。
- entitlement access 只查询一次，scope/model/token policy 决策一致。
- active entitlement 的 `medcode_models.allowed` 即使不包含 `medcode`，`model=medcode` 也继续通过兼容门禁。
...
- OpenRouter adapter：
  ...
  - GLM / DeepSeek 发送 `reasoning: { "effort": "none" }` 后，小 `max_tokens` 下不会返回 reasoning 截断提示，且 `reasoning_tokens=0`。
...
4. `POST /v1/chat/completions model=standard`，固定词返回。
5. `POST /v1/chat/completions model=pro`，固定词返回。
6. streaming smoke，确认 `data: [DONE]`。
7. strict tools smoke，确认返回 `tool_calls`，再用 `role=tool` follow-up。
8. entitlement smoke，低档 key 调 Max 返回结构化权限错误。
9. token budget smoke，超限请求被拦截。
10. request_events 检查 public model、runtime、upstream model、token usage。
11. identity guard smoke，确认普通对话和 strict tools 路径不泄漏真实上游。

### Desktop E2E

- Provider 设置页仍只出现 MedCode。
- 模型选择器显示 Max / Pro / Standard。
- 默认模型仍是 `medcode/medcode`。
- Provider Test 只验证 credential，不调用具体模型。
- 选择 Pro / Standard 后能完成普通对话。
- 选择 Pro / Standard 后医学问题仍能触发 `medevidence` 工具。
...
如果 Desktop 已发布三档模型，Gateway 不应删除 public model id。必要时把 `pro` / `standard` 临时标为维护中，或临时路由到 `medcode`，避免用户看到不可解释的 `model_not_found`。

## 风险和审核点

1. 数据流向和合规
   Pro / Standard 会把用户 prompt 发往 OpenRouter，再由 OpenRouter 路由到 Z.AI / DeepSeek 或其托管 provider。需要确认是否符合用户告知和数据处理边界。
...
5. 附件能力
   OpenRouter 当前 `z-ai/glm-5.2` 和 `deepseek/deepseek-v4-pro` 目录显示为 text-to-text。Pro / Standard 不应声明 PDF / file 输入，除非 Gateway 先做附件文本化。

6. 产品命名冲突
   我们的 Max / Pro / Standard 是 MedCode 档位；Z.AI 自己也有 Coding Plan Lite / Pro / Max。对外文案需要避免混淆。
...
6. 新增 entitlement `feature_policy.medcode_models` schema、parser、serializer、entitlement snapshot 和 admin / billing 输出。
7. 抽出 entitlement access helper，统一处理 scope、model allow-list 和 token policy，避免重复查 entitlement。
