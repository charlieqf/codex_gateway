...
Agent 也需要可管理的来源工作区。仅保留最近工具结果和文字笔记，会使作者归属材料与核心摘要互相挤出，造成重复阅读。当前允许 Agent 自选并持续保留已读论文的摘要、作者归属或完整视图；代码只做来源投影、去重、容量限制及持久化。最终复核接收完整选定摘要，不能在中间静默截断。这项改进的真实模型效果仍待验证。
