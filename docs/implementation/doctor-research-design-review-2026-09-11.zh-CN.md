...
**范围更新（2026-09-11）**：用户已明确这是医生资料搜索任务，产出不要求很高学术性。后续应按[实用资料交付范围评估](doctor-research-practical-profile-scope-2026-09-11.zh-CN.md)调整默认合同；以下描述保留为旧学术流程的实现与诊断记录，不能把其 5000 字、40 篇文献及多轮学术复核继续视为默认用户需求。
...
Agent 也需要可管理的来源工作区。仅保留最近工具结果和文字笔记，会使作者归属材料与核心摘要互相挤出，造成重复阅读。当前允许 Agent 自选并持续保留已读论文的摘要、作者归属或完整视图；代码只做来源投影、去重、容量限制及持久化。canary9 已实际使用并进入提案阶段，但尚未完成交付。最终复核接收完整选定摘要，不能在中间静默截断。

工具反馈同样影响效率。每次只暴露第一处格式或引用错误，会让 Agent 多次往返才能知道完整问题。当前一次收集各条记录的机械校验问题，并支持无歧义的提案外层路径前缀，修正后仍须完整校验和独立复核。结构化输出约束尚未贯通至模型提供商，普通消息中的 JSON 格式错误仍是待改进环节。
