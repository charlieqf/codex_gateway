...
状态：已按用户要求实施、自审、自测、部署及线上自测；运行提交 `71689e3`，schema 34，于 2026-09-18 08:36 UTC 生效。本文保留设计范围与门槛；实际测试、容量边界、发布及回滚证据见[验收记录](../operations/r760-identity-request-audit-release-2026-09-18.zh-CN.md)。

改造前基线：`1a37c0c`。第 2 节的“现状”以该已提交版本的改造前职责为准。发布时已 fetch 并核对唯一工作区 main、origin/main 和生产基线；无关工作区改动未纳入发布。

评审重点：职责拆分、完整手机号的受控留存、429 聚合的信息损失，以及明确的替换／删除清单。实施采用本文的 30/7 天留存和正常流量性能门槛；具体压测负载、清理上限及告警沿用边界见验收记录。第 14 节保留为团队评审清单，不代表已获得每位审核者签署。
...
以下符号以 `1a37c0c` 为基线；文件链接指向工作区，相关旧方法已被新实现替换。复核原职责时使用 `git show 1a37c0c:<文件路径>`，不将新旧实现的描述混为一谈。
