...
- Gateway request context 已承载 dev credential、subject、upstream account、provider 和 scope，为 Phase 2 credential lookup 替换做好入口。
...
- SQLite store 已有幂等 schema migration，覆盖 `subjects`、`upstream_accounts`、`access_credentials`、`sessions`、`request_events` 基础表。
...
- Gateway 设置 `GATEWAY_SQLITE_PATH` 后会使用 SQLite Session Store，并在启动时 seed 开发 subject/upstream account。
...
- 上游账号配置内部仍主要依赖 bootstrap `upstream_accounts`；更完整的上游账号管理和管理员身份记录仍待实现。
