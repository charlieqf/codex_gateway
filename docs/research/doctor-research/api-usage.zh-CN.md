...
有界 GET 重试、严格 manifest 校验、同源认证下载、SHA-256 校验和原子发布。推荐从
JSON 请求文件调用：
...
不能通过客户端重试、删除诊断或放宽校验把失败包装为成功。医学团队维护的 Skill 是
业务权威来源；每主题 600 字等软门槛、代表病例接受标准和最终四文件内容验收均由医学
团队决定。自动化和 SHA-256 通过只证明工程完整性，不替代医学内容人工审核。
