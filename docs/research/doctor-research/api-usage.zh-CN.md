...
不代表该医生本人的研究成果或观点。身份三锚点本身仍按原门槛核验。
...
正在实施、尚未部署的 Agent 流程增加 `resource_budget_exceeded`，表示搜索、读取、模型调用、
token 或状态存储预算耗尽；它不同于“没有搜到医生”，也不同于模型输出格式错误。
客户端应显示服务返回的公开说明并保留 `request_id`，不要因该错误自动创建大量新任务。
`deadline_exceeded` 继续单独表示整体时间上限。该新增终态不改变查询或幂等协议。
...
服务体验按完整任务墙钟耗时评估：创建到可用结果不超过 5 分钟为优秀，超过 5 分钟且
不超过 8 分钟为可接受，超过 8 分钟需要检查排队、上游请求、模型生成和修复耗时。
这不是单独身份核验阶段的标准，也不表示到了 8 分钟就自动取消任务。
