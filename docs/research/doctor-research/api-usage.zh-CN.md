...
经过身份、医院、科室、研究方向和文献作者归属硬验证。自 `1.6.79` 起，调用方可选提供的
`official_profile_urls` 只会补充、不会替换服务端已审核来源，并继续去重且最多保留三条。
已注册医生建议只提交三个必填字段，避免客户端长期缓存身份来源。三字段只减少用户输入，
不降低身份门槛。尚未注册且调用方又未提供增强身份信息的医生可能终止为
...
真实用户 run `drr_32fe62a652dc4a9f8d9b561b68a478e5` 随后暴露旧版问题：客户端实际
携带的一条可选官方 URL 覆盖了服务端完整来源集合，使中英文身份桥接证据丢失并终止为
`identity_not_resolved`。这不是状态查询失败，也不是部署未生效。`1.6.79` 修复合并语义后，
顶层仍恰好只传三个必填字段的公网 run `drr_07b07f128ce746edb777fbc70dbe3340`
在服务端 265.857 秒内成功，恰好 3 MD + 1 TXT 及全部 manifest、大小和 SHA-256 校验通过。
...
`1.6.83` 修复了另一种服务端内部路由边界：一次有界 QA 修正完成后，如果完整验证只剩
一个可定向修复的章节，第五次也是最后一次模型调用会使用章节 ID、原文 SHA-256 和允许
evidence ID 绑定的单章节修复，而不再浪费在无法利用该结构化诊断的通用 peer 调用上。
客户端接口、三个必填字段、幂等语义和质量门槛均未改变。发布后本 Python 示例完成公网
run `drr_ad1f050c609945c29d546315d4857173`：服务端 201.212 秒、客户端 203 秒成功，
并逐项验证恰好 3 MD + 1 五行 TXT、manifest、文件大小和全部 SHA-256。
...
业务权威来源。生产 `1.6.79` 受控试用策略继续按原目标生成，但把纯篇幅完整度拆成
