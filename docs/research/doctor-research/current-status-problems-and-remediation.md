...
| 任意医生身份发现 | 当前 `1.6.83` 仍为 `direct`，审核注册表只有 1 位工程病例；本地候选 `1.6.84` 已完成通用公网发现和无本人论文路径 | **尚未上线**；注册表已改为仅作缓存，仍需安装搜索凭据并通过未注册医生公网 E2E |
...
本地 `1.6.84` 候选已经形成三个可独立回滚的提交：`170766d` 把 Brave 路径改为有界通用
身份搜索并继续执行公网地址钉定和三锚点验证，`e7a38b1` 使生产在缺少通用搜索配置时
拒绝启动，`a7caca8` 则取消“本人至少 3 篇 PubMed 论文才能研究”的工程假设。医生本人
论文现在只用于可选代表作；没有本人论文时代表作保持为空并返回
`doctor_publication_evidence_not_found`，独立的领域文献最少数量和全部医学质量门槛不变。
完整 `npm run build`、40 个 Vitest 文件共 600 项、36 个 Python 测试和
`git diff --check` 已经通过，医学团队 Skill 零 diff；这仍是本地候选证据，不能替代
Azure 部署和未注册医生真实公网验收。VM 只读预检同时确认当前 Worker 没有可读的
Brave Search secret，因此在凭据安装前，生产会安全保持 `1.6.83`，候选代码会按设计拒绝
以 `direct` 模式伪装成通用搜索版本启动。
