...
`/home/qian/codex-gateway-release-29790d2-20260730T062157Z`。Prompt 为 `v30`，
validation 为 `v42`，workflow 为 `v77`。四个生产容器均 healthy、重启次数 0；只有
Gateway 发布 `127.0.0.1:18787`，公网和 loopback 均为 `ready / controlled-trial`。

当前工程验收已非常接近完成：取消传播、无重叠重试、全调用时间线、离线回放、统一规则源、
hash-bound 单章节修复、证据投影、四文件原子提交和任意医生公网发现均已上线。完整门禁通过
40 个 Vitest 文件/625 个测试、36 个 Python 测试、零漏洞审计、`git diff --check` 和医学
Skill 零 diff。`1.6.99` 对同一未登记工程病例连续 5/5 次公网成功，耗时 177.257–257.217
秒；每次都是恰好 3 MD + 1 五行 TXT，manifest、大小、SHA-256 和时间线全部一致。

用户提供的“陆清声 / 海军军医大学第一附属医院 / 血管外科”三字段病例进一步暴露并验证了
真实覆盖：旧查询把论文标题中的高频连接词 `for/and` 误当作 PubMed 主题词，因而错误返回
`insufficient_research_evidence`。`1.6.100` 在主题抽取和查询构造两层过滤连接词；发布后两次
都完成身份核验和 40 篇字段文献闭合。第一次在 147.124 秒因两个独立模型质量门槛同时失败而
正确零产物终止；第二次 `drr_acadf775c00c42c1924ebf3180a519b7` 在 173.301 秒成功并通过
四文件严格校验。这证明任意医生发现和该病例可以走通，也如实显示模型质量通过率还不是 100%。

| 事项 | 当前结论 |
| --- | --- |
| 搜索覆盖 | SerpAPI Google 已上线；注册表只是缓存，不再要求医生预先收录 |
| 准入配置 | 每 subject 每 UTC 日 50 次；不限制不同医生数量；单 active run 和队列限制保留 |
| 稳定性 | 同一工程病例 5/5；新增真实病例在搜索修复后 1/2 通过完整质量合同、2/2 在 10 分钟内终止 |
| 取消与资源 | chat/responses 非流式断开均到达 provider，`cancel_requested/observed=1/1`；无旧新调用重叠，资源归零 |
| 回滚与备份 | 新边界 `/home/qian/codex-gateway-backups/29790d2/20260730T062157Z` 和四个 `rollback-cb703da-*` 镜像已验证；发布后备份 `drb_ad694a206a6746d8b1f95a1a07741b72` 通过无网络只读复验 |
| 磁盘 | 已从约 17 GiB 可用恢复到约 24–26 GiB；新发布和备份完成后的最终值为约 24 GiB，当前卷、当前发布和有效回滚边界均保留 |
| 临时资源 | E2E key 已撤销、用户已禁用、entitlement 已取消、临时文件和脚本已删除 |
| 正式放量 | **尚不可放量**；唯一剩余外部门槛是医学团队确认代表性病例、审核四文件内容并决定软完整度策略 |

因此当前最准确的可用性判断是：基础设施、搜索、取消、遥测、产物完整性和 10 分钟终态约束已
达到受限试用要求；随机模型输出仍可能因硬质量合同 fail-closed。工程侧不能通过降低身份、引用、
数值、证据等级、安全或四文件门槛掩盖失败。在医学团队完成人工验收前继续保持 controlled-trial。

以下章节保留此前各版本整改过程和历史生产快照，凡与本节冲突均以本节为准。

## 一、历史整改摘要（截至 1.6.83）

Doctor Research API 的 `1.6.83` 已部署到 Azure VM 的公网生产入口。
...
确认该病例的正式代表性，也未完成人工四文件内容验收，因此仍定义为**受限试用版**，
不能扩大用户范围。
...
| 当前版本 | commit `ddb1dcca5a92d2d032383f9cb01ae5cf65b22be4`，执行器 `1.6.83` | 已上线 |
| 运行时上限 | 服务端硬截止 570 秒，客户端发布验证最多等待 590 秒 | 满足“整体不超过 10 分钟”的原则 |
| 自动化验证 | 已部署 release 在本地和 Azure 通过 build、40 个测试文件共 598 个 Vitest、36 个 Python 测试；npm audit 为 0，`git diff --check` 通过 | 工程回归通过 |
| 每日准入额度 | 每个 subject 每个 UTC 自然日 50 次；当日已准入失败/取消仍计数，幂等重放不重复计数 | 已上线；不改变并发、队列、身份、证据和四文件门槛 |
| 不同医生数量 | 不限制；四服务显式加载 `RESEARCH_MAX_UNIQUE_DOCTORS_PER_SUBJECT_30D=0` | 已按业务负责人授权上线；每日 50 run 和其他门槛仍独立执行 |
| 任意医生身份发现 | 当前 `1.6.83` 仍为 `direct`，审核注册表只有 1 位工程病例；本地候选 `1.6.84` 已完成通用公网发现、SerpAPI provider 和无本人论文路径 | **尚未上线**；Worker-only SerpAPI secret 已安全安装但未被旧容器挂载，仍需部署并通过未注册医生公网 E2E |
...
今天已经完成的是“把整改闭环部署到 Azure，用 5 次连续真实运行证明同一缓存病例的稳定性，并进一步
验证该病例只提交姓名、医院和科室也能在 10 分钟内原子发布完整四文件”。这不能证明任意医生可搜索；
现网注册表只有该工程病例，且 `direct` 模式不会为未登记医生执行开放搜索。尚未完成的是通用身份发现上线、医学团队确认病例代表性、审核成功四文件，
以及决定当前受控软完整度发布线是否可成为长期业务政策。多个独立硬错误后的额外改写
仍保持 fail-closed，不作为提高成功率的手段。

本地 `1.6.84` 候选已经形成四个可独立回滚的提交：`170766d` 把 Brave 路径改为有界通用
身份搜索并继续执行公网地址钉定和三锚点验证，`e7a38b1` 使生产在缺少通用搜索配置时
拒绝启动，`a7caca8` 取消“本人至少 3 篇 PubMed 论文才能研究”的工程假设，`c03c14d`
则新增可显式选择 Google/Baidu 引擎的 SerpAPI provider，并保持 Brave 为可回滚选项。医生本人
论文现在只用于可选代表作；没有本人论文时代表作保持为空并返回
`doctor_publication_evidence_not_found`，独立的领域文献最少数量和全部医学质量门槛不变。
同一未登记工程病例的受限 provider 探测中，SerpAPI Google 返回 9 个自然结果，但前五项
没有医院官网；Baidu 返回 3 项，第一项为瑞金医院官网，且前五范围内有 2 个姓名命中的
HTTPS 候选，因此当前生产候选显式选择 `RESEARCH_SERPAPI_ENGINE=baidu`。搜索结果仍只作
候选，后续公网地址钉定、正文抓取和姓名/医院/科室三锚点验证完全不变。

完整 `npm run build`、40 个 Vitest 文件共 603 项、36 个 Python 测试、npm audit 和
`git diff --check` 已经通过，医学团队 Skill 零 diff；这仍是本地候选证据，不能替代
Azure 部署和未注册医生真实公网验收。SerpAPI 账户状态和额度已验证，Worker-only secret
已经以 `999:999 / 0400` 安装到当前发布目录，但 `1.6.83` Compose 不挂载也不读取它，
所以生产仍安全保持旧行为，尚未发生服务重启或流量切换。
...
但它们和身份、引用、数值安全不是同一类风险。当前生产把原值保留为生成目标，另设
受控试用最低发布线：总正文 5000、引言 640、每主题 450、综合 640、局限 450、结论
160。达到发布线但低于目标的结果必须标记 `passed_with_warnings`；低于发布线仍失败。
这项可回滚工程策略已经通过回放和 5/5 生产验证，但是否长期采用或继续调整仍由医学
团队决定，不能为了单次通过把 `600` 临时改成任意更低值。
...
- Prompt：`v29`
- Validation：`v42`
- Workflow：`doctor_research_workflow.v72`
...
Worker 报告 `doctor-research-skill.1.6.83`，内部 LLM Gateway 截止为 175000 ms。
四个容器实际加载的 `RESEARCH_MAX_DAILY_RUNS_PER_SUBJECT` 均为 `50`，且
`RESEARCH_MAX_UNIQUE_DOCTORS_PER_SUBJECT_30D` 均为 `0`。发布后公网
...
第 6 位，因此在模型调用前被拒绝。`ff5db5f` 当时保留该边界，并把
...
随后业务负责人明确决定不限制不同医生数量。`ddb1dcc` 用显式 `0` 禁用该准入检查，
同时保留正整数回滚行为，并继续对缺失、负数、小数和不可解析配置 fail-closed。公网
请求 `req-a420c2e0-49fc-49b3-8190-eebe1d17b54a` 成功创建此前会被视为新医生的 run
`drr_44172c711e494cacb3b0eda1947326a7`；验收随即取消该 run，终态为 `cancelled`、
artifact 为 0，且恰好新增一条 run/admission，临时 key 已撤销。
...
`ff5db5f` 四个镜像已打上 `rollback-ff5db5f-20260729T063301Z` 标签。数据库哈希分别为
Gateway `43a4f070...244a`、client-events `c9b4aac7...0742`、Research
`5bc65595...7d92`、Research LLM `75d5570a...2ca7`，完整值见生产 runbook。
...
- 当前版本已提供完整中文 API 调用/注意事项、只含三个必填业务字段的 JSON 请求样例和
  Python `--request-file` 客户端。

暂时不能承诺：

- 当前生产已经支持任意医生；`1.6.83` 仍没有通用搜索凭据和通用发现运行时；
- 所有医生、所有上游状态下的每次请求都一定成功；即使 `1.6.84` 上线，公开身份或领域
  证据不足仍会诚实失败，当前稳定性证据仍是单一工程病例连续 5/5；
- `1.6.83` 生成的 3 MD + 1 TXT 已经完成人工医学内容验收；
- 可以无风险扩大到普通用户或高并发流量。
...
| `1.6.75 @ ff6980d` / `drr_bfd08d458dea4928a0b41a246a6f3973` | `model_contract_error`，零 artifact | 连续五次窗口中的唯一失败；两次正文分片返回合法内容但多包一层对象，暴露 body fragment envelope 契约歧义，其余 4 次成功 |
| `1.6.76 @ 638e51d` / `drr_701cffeff8534a4699a255556e2828eb` | 成功，327.055 秒 | 严格 Python 公网 E2E；恰好四文件，manifest、大小和 SHA-256 一致 |
| `1.6.76 @ 638e51d` / `drr_6e631e5255254b5ca479a91bdfdff8d9` | 成功，378.099 秒 | provider 截止后确认取消，下一 attempt 在旧调用终止后才开始；四文件一致 |
| `1.6.76 @ 638e51d` / `drr_b789d7d82b974872a59d3b784e9b1d6d` | 成功，321.301 秒 | 严格 Python 公网 E2E；四文件一致 |
| `1.6.76 @ 638e51d` / `drr_0ac3161cfb0d47a38194e2b714d45726` | 成功，344.805 秒 | provider 截止后确认取消，下一 attempt 严格顺序启动；四文件一致 |
| `1.6.76 @ 638e51d` / `drr_38ff2d2cfadd4bbb9043551acc29d762` | 成功，166.765 秒 | 严格 Python 公网 E2E；四文件一致 |
| `1.6.77 @ c439848` / `drr_1e696a86a85849278d4023b255197967` | 成功，320.668 秒 | 请求正文顶层恰好 `name/hospital/department`；服务端审核身份注册表补足闭合来源和英文文献身份；15/15 阶段、3 MD + 1 TXT、五行 TXT、manifest、大小和 SHA-256 全部通过 |
| `1.6.77 @ c439848` / `drr_32fe62a652dc4a9f8d9b561b68a478e5` | `identity_not_resolved`，约 1.079 秒 | 客户端实际提交了单条 `official_profile_urls`；旧合并语义覆盖服务端两条审核来源，导致中英文姓名桥接来源丢失，身份门槛按设计 fail-closed；不是查询接口或部署失败 |
...
| `1.6.82 @ 02b74de` / `drr_cd3716ae58524bf299e36d6437b12a00` | `model_contract_error`，143.414 秒，零 artifact | 五次 Aliyun 调用均 200；QA 修正成功后，第五次仍用于通用 peer 且返回契约不可用，确定性回退只剩一个主题 `313/450`。根因是末次调用路由缺口，不是身份、额度、部署或 provider 故障 |
| `1.6.83 @ eb94fa8` / `drr_ad1f050c609945c29d546315d4857173` | 成功，服务端 201.212 秒、客户端 203 秒 | 保持五调用上限和全部硬门槛；三字段公网 E2E 的五次 Aliyun 调用均 200、无取消或后续调用重叠，76 sources、40 references、9 claims 和恰好 3 MD + 1 五行 TXT 的 result/manifest/file 大小及 SHA-256 全部一致 |
| `1.6.99 @ cb703da` / 五次连续工程 E2E | 5/5 成功，177.257–257.217 秒 | 同一未登记工程病例每次恰好 3 MD + 1 五行 TXT；manifest、大小、SHA-256、Worker/Gateway/provider 时间线和资源归零全部通过 |
| `1.6.99 @ cb703da` / `drr_48200686d0d54803b47ea0505989dd8f` | `insufficient_research_evidence`，零 artifact | 真实三字段病例身份和 5 篇本人论文均核验成功，但论文标题高频连接词 `for` 进入前三个字段检索词，形成无意义的过窄查询；不是未收录或身份失败 |
| `1.6.100 @ 29790d2` / `drr_2a54d1eca9094d90a7752878fe7ae3d7` | `model_contract_error`，147.124 秒，零 artifact | 搜索修复后闭合 40 篇字段文献；三个初始模型调用均 200，但一个正文主题低于 450 发布线且结尾结构合同同时失败，按多独立门槛策略 fail-closed |
| `1.6.100 @ 29790d2` / `drr_acadf775c00c42c1924ebf3180a519b7` | 成功，173.301 秒 | 同一真实三字段病例闭合 40 篇字段文献；五次调用均为 provider response、取消 0/0、资源归零；恰好 3 MD + 1 五行 TXT，大小和 SHA-256 全部一致 |

发布前连续五次和发布后精确 runtime E2E 均使用当前工程 allowlist 的同一 smoke 病例；医学团队仍需明确确认它是否可作为
正式代表性病例，并补充其要求覆盖的其他病例。该组数据可作为工程基线，不能替代医学
团队对病例代表性和四文件内容的判断。

同口径历史比较中，`1.6.58` 两次运行的平均终态时间为 431.734 秒，`1.6.72` 发布前五次为
270.852 秒，下降约 37.3%。当前 `1.6.76` 五次均成功，客户端耗时平均 307.605 秒，范围
166.765–378.099 秒。关联 Gateway 事件的单次估算输入 token 均值曾从
14116.89 降到 12635.76，下降约 10.5%。这是跨版本、不同调用组合的生产观测，不是隔离
因果实验；它证明 token/墙钟方向已经改善，但不能替代回放零回退、真实成功率和医学
人工内容质量验收。

以下早期五个成功 run 的 manifest/download SHA-256 记录如下，列顺序为
...
- 建立 15 个脱敏离线回放 fixture、21 个回放测试，并保证重复运行诊断、语义和 artifact hash 确定；
...
- 只对唯一的常规 `body`/`body_fragment`/`review` envelope 做有界机械解包，并在解包后
  重跑全部 fragment、Skill 和完整输出验证；歧义或缺字段仍 fail-closed。
...
生产 smoke 中，Chat Completions 与 Responses 非流式调用均在约 2 秒断开后记录
`client_aborted`、`terminal_source=client_abort`、`cancel_requested=1` 和
`cancel_observed=1`，无服务重启和未结算 reservation。正常非流式
...
fail-closed。15 个独立 replay fixture 通过 21 个测试覆盖历史缺陷、provider 错误和确定性重复运行，
...
| 项目 | 实际状态 | 对后续方案的影响 |
| --- | --- | --- |
| 模型输出回放 | 独立 replay 目录已有 15 个脱敏 fixture、21 个测试，使用固定证据、可注入时钟和语义/hash 断言 | 规则和修复改造受回放保护；known-invalid 继续隔离 |
...
`1.6.76` 的自动化、取消和连续 5/5 次公网四文件验证已经通过。由于该样本仍是同一工程
allowlist 病例，不能据此推断所有医生或医学内容都已验收。当前验收仍需要同时覆盖：
...
- Azure 磁盘不足：部署通过 10 GiB 和 10% 双重磁盘准入线；
- Gateway/LLM Gateway 容器崩溃：核心容器保持 healthy，LLM Gateway 为 0 重启；
- 医学 Skill 原始目录被修改：原始目录零 diff，bundle 摘要一致；
- API 无限等待：五次 `1.6.76` 运行均在 10 分钟前成功终止；
- 系统发布半成品：此前失败 run 为 0 artifact，当前五个成功 run 均恰好四个。

## 六、具体优化方案

下列 6.2 至 6.7 的工程工作已经在 `1.6.76` 落地并通过自动化与生产验证；保留具体条目
作为设计与防回退依据。6.9 的模型/provider 评估尚未启动；6.10 的受控软完整度发布线
已经上线并有 5/5 工程证据，其长期业务接受仍需医学团队决定。
...
2026-07-23 的当前任务方向明确要求优先提高服务成功率和稳定性，让受限用户先顺畅使用。
据此，当前生产策略没有修改医学 Skill，也没有降低身份、引用、数值、证据等级、安全、
章节数量、5 问 5 答或四文件完整性门槛，而是把原篇幅值保留为 Prompt 生成目标，并增加
可回滚的 `controlled-trial` 最低发布线：正文合计 5000、引言 640、每主题 450、证据综合
640、局限 450、结论 160。低于原目标但达到发布线时必须返回
`controlled_trial_*_below_target` warning；低于发布线、缺章节或任一硬门槛失败时仍为零
artifact fail-closed。

离线回放已覆盖两个 476 字主题且总正文低于 6000 的受控边界样例：它可确定性成功并返回
正文及主题 warning；原短主题负例以及引用、数值、因果、残句、500、429、超时和取消
fixture 继续保持原终态。该策略只用于受控试用，不代表医学人工内容验收完成，也不授权
扩大用户范围。`1.6.76` 上线后同一工程病例连续 5/5 成功，结果均为
`passed_with_warnings` 并通过四文件完整性验证；医学团队仍需判断是否接受这些带软篇幅
warning 的产物。该数据支持保留当前发布线，不支持在缺少医学决定和多病例数据时继续下调。
...
1. `1.6.100` 已完成搜索凭据、备份、部署和未注册医生公网 E2E；在医学团队完成代表病例和
   四文件人工验收前仍保持受限试用，不扩大用户范围。
...
1. 医学团队确认当前“原目标 + controlled-trial 发布线 + warning”是否可长期采用。
2. 医学团队确认当前工程病例是否有代表性，并提供其他代表病例和人工验收清单。
...
支持对任意三字段医生发起 SerpAPI Google 公网发现，具备 10 分钟硬截止、provider 取消、
可还原时间线和医学质量 fail-closed 保护；工程病例连续 5/5 成功，新增真实病例也已完成
一次严格四文件成功，但模型质量合同仍可能诚实拒绝不合格输出，且病例代表性和四文件内容
尚未通过医学团队人工验收。**
