...
- 扩充经过预审的证据边界段落池；
- 把展示清理改成最多 8 次的有界 fixed point，并补齐当轮已知规则；但长度、段落分类和
  孤立指代的规则源仍未统一，不能把这项修复描述为结构性问题已经解决。
...
### 5.2 P0 工程缺口：非流式客户端断开没有传播到 provider
...
3. Worker 对失败分片发起重试；
4. 新旧调用重叠，进一步放大长尾、429、500 和费用不确定性。

现有证据还不能证明每次超时都留下了旧调用，但代码没有提供“旧调用已经取消”的保证。
在完成取消传播和相应回归前，不应把所有长尾都归因给模型或供应商。
...
### 5.4 工程结构问题：规范化、验证和 Prompt 规则分散

当前 600 字、800 字、200 字等章节规则同时出现在 Prompt、分片验证、完整综述验证和
补齐器中；孤立指代规则也存在宽窄不同的多份实现。`1.6.57` 直接暴露了规范化器与最终
验证器的覆盖范围漂移。`1.6.56` 的 `588/600` 更准确地说是边界补齐能力不足；两者都
属于规则体系脆弱，但不是完全相同的根因。

`1.6.58` 的有界 fixed point 只解决了当轮已知展示清理的收敛问题，没有把规则变成单一
来源。继续增加新的句式补丁会重复此前的失败模式。

### 5.5 代码现状需要准确描述

| 项目 | 实际状态 | 对后续方案的影响 |
| --- | --- | --- |
| 模型输出回放 | 尚不存在；现有测试主要使用手写合成响应 | 应建设，但不能直接把历史最终四文件当成原始响应 |
| `known-invalid` 样本 | 是隔离的历史展示输出，目录标记明确禁止作为 golden、benchmark 或 executable fixture | 只能用作缺陷目录和派生负例来源，原目录继续保持隔离 |
| 分片指标 | `research_stage_runs` 已有总时长、token、request ID 和错误；没有 admission wait 或真正 provider 首事件 | 需要跨 Worker/Gateway 的统一调用时间线，而不是只加一个 `TTFB` 字段 |
...
| 定向修复 | peer review 已使用 `old_text/new_text` 补丁，QA、引言和结论也有定向修复 | 剩余主要问题是“可解析但分片契约失败”仍会整分片重写 |
...
优化顺序应是：先阻止旧调用与重试重叠，同时建立可解释的调用时间线；并行建设离线
回放；然后统一规则和缩小修复范围；最后才根据数据优化 Prompt 或评估其他 provider。
...
### 阶段 D：数据驱动的 Prompt 与 provider 优化
