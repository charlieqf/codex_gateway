...
top-level `unified_key` in `cmev1` form:

  cmev1.<codex-gateway-api-key>.<medevidence-v2-api-key>

Use -UnifiedKeyMode cmev1 for the current Desktop handoff. -UnifiedKeyMode
opaque is reserved for a future design and is blocked here; this script only
creates the current cmev1 wrapper from a Gateway key and a MedEvidence key.
...
param(
  ...
  [Alias("UnifiedKeyVersion")]
  [ValidateSet("cmev1", "opaque")]
...
function New-UnifiedKey {
  ...
  if ($Mode -ne "cmev1") {
    throw "UnifiedKeyMode '$Mode' cannot be generated by this Gateway provisioning script. This script only creates the current cmev1 wrapper from a Gateway key and a MedEvidence key."
  }
  ...
  "$Mode.$CodexGatewayToken.$MedEvidenceV2Token"
}

function Get-MedEvidenceV2Token {
  param(
  ...
  if ($Entry.PSObject.Properties["plaintext_api_key"]) {
    $token = [string]$Entry.plaintext_api_key
    if (-not [string]::IsNullOrWhiteSpace($token)) {
      return $token
    }
  }

  if (-not $Entry.PSObject.Properties["api_keys"]) {
    return $null
  }

  $candidateKeys = @(@($Entry.api_keys) | Where-Object {
    $_.PSObject.Properties["plaintext_api_key"] -and
      -not [string]::IsNullOrWhiteSpace([string]$_.plaintext_api_key) -and
      (
  ...
  if ($candidateKeys.Count -eq 1) {
    return [string]$candidateKeys[0].plaintext_api_key
  }
  if ($candidateKeys.Count -gt 1) {
    throw "Found multiple active nested plaintext_api_key values in $Path; cannot choose a MedEvidence key safely."
  }
...
$medEvidenceV2Token = Get-MedEvidenceV2Token -Entry $entry -Path $resolvedPath
if (-not $SkipUnifiedKey -and [string]::IsNullOrWhiteSpace($medEvidenceV2Token)) {
  throw "Missing plaintext_api_key in $resolvedPath; cannot build unified_key."
}
...
if (-not $PSCmdlet.ShouldProcess($UserId, "provision Codex Gateway API key and update $resolvedPath")) {
  [pscustomobject]@{
    ...
    unified_key_mode = $(if ($SkipUnifiedKey) { $null } else { $UnifiedKeyMode })
...
if (-not $SkipUnifiedKey -and $UnifiedKeyMode -eq "opaque") {
  throw "UnifiedKeyMode 'opaque' is intentionally blocked in this Gateway provisioning script. This script only creates the current cmev1 wrapper from a Gateway key and a MedEvidence key. Use -UnifiedKeyMode cmev1 for the current Desktop handoff."
}
...
if (-not $SkipUnifiedKey) {
  ...
  $entry | Add-Member -Force -NotePropertyName unified_key_version -NotePropertyValue $UnifiedKeyMode
