...
const summaryFor = s => ({
  ...
  auth_history: db.prepare('SELECT action,outcome,reason_code,count(*) AS count,min(created_at) AS first,max(created_at) AS latest FROM phone_auth_audit_events WHERE subject_id=? GROUP BY action,outcome,reason_code').all(s.id),
...
const errorReasons = ['phone_identity_conflict', 'account_migration_required', 'subject_mismatch'];
const placeholders = errorReasons.map(() => '?').join(',');
const errors = db.prepare(`SELECT subject_id,action,reason_code,count(*) AS count,min(created_at) AS first,max(created_at) AS latest FROM phone_auth_audit_events WHERE reason_code IN (${placeholders}) GROUP BY subject_id,action,reason_code`).all(...errorReasons);
...
const windowSummary = since => db.prepare('SELECT action,outcome,reason_code,count(*) AS count,count(DISTINCT subject_id) AS subjects,min(created_at) AS first,max(created_at) AS latest FROM phone_auth_audit_events WHERE created_at>=? GROUP BY action,outcome,reason_code').all(since);
console.log(JSON.stringify({
  ...
  audit_retained_range: db.prepare('SELECT count(*) AS count,min(created_at) AS first,max(created_at) AS latest FROM phone_auth_audit_events').get(),
