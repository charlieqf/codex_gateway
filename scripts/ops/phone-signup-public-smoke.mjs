...
try {
  ...
  assert.equal(ready.current.identity.plan_id, "plan_free_daily_100k_v1");
  assert.equal(ready.credential.credential.token.tokensPerDay, 100_000);
  ...
  report.new_account = { subject_id: fresh.subjectId, plan_id: ready.current.identity.plan_id,
    daily_tokens: 100_000, no_resolve: true, no_name: true, same_key_on_relogin: true, grant_count: 1, model_http_status: 200 };
  ...
  assert.equal(twoReady.current.identity.plan_id, "plan_free_daily_100k_v1");
  assert.equal(twoReady.credential.credential.token.tokensPerDay, 100_000);
  report.optional_resolve = { subject_id: twoStep.subjectId, two_step_supported: true, daily_tokens: 100_000 };
