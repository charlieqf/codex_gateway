#!/usr/bin/env python3
"""Activate tested Gateway A/S for one synthetic smoke subject; schema remains 30.
...
canary=sys.argv[2]
assert re.fullmatch(r'subj_[A-Za-z0-9_-]{8,80}',canary)
assert len(sys.argv)==3 or sys.argv[3:]==['--restart-with-active-requests']
restart_with_active_requests = len(sys.argv)==4
flags={'GATEWAY_BOUNDED_WRITE_MODE':'delivery','GATEWAY_BOUNDED_WRITE_SUBJECT_IDS':canary,'GATEWAY_WRITE_DELIVERY_MAX_CONCURRENT':'2'}
...
assert not set(flags)&set(expected_env), 'Unexpected existing A/S configuration'
...
assert count==1
assert not any(key in block for key in flags)
...
with readonly(dbpath) as db:
    ...
    smoke_subject=db.execute('SELECT external_user_id,state FROM subjects WHERE id=?',(canary,)).fetchone()
    assert smoke_subject and smoke_subject[0].startswith('bounded_write_smoke_') and smoke_subject[1]=='active', 'Active synthetic smoke subject required'
    last_pending=None
...
try:
    ...
    state.update(deployed_at=datetime.datetime.now(datetime.timezone.utc).isoformat(),gateway_started_at=current['State']['StartedAt'],
        gateway_container_id=current['Id'],expected_env_sha256=expected_env_sha,bounded_write_canary=canary,bounded_write_flags=flags,
    ...
    emit(event='activated',revision=rev,image_id=current['Image'],health='healthy',restarts=0,
        other_containers_unchanged=True,bounded_write_mode='delivery',canary_subject=canary,backup=str(backup),deployed_at=state['deployed_at'])
finally:
    if changed:
