#!/usr/bin/env python3
"""Reconcile Azure Gateway control state into R760 without overwriting its DB."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Safely reconcile Azure's Gateway control-plane rows into R760. "
            "The default is a read-only dry run; --apply creates a verified R760 backup first."
        )
    )
    ...
    parser.add_argument(
        "--fail-on-drift",
        action="store_true",
        help="For scheduled dry-runs, return exit code 3 when reconciliation is required.",
    )
