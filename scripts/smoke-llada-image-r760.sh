...
smoke_user=llada-image-smoke-$timestamp
tmp=$(mktemp -d /data/codex-gateway-r760/llada-image-smoke.XXXXXX)
chmod 0700 "$tmp"
prefix=
token=
stage=initialize
...
cleanup() {
  ...
  case "$tmp" in
    /data/codex-gateway-r760/llada-image-smoke.*) rm -rf -- "$tmp" ;;
...
admin issue \
  --user "$smoke_user" \
  --user-label "LLaDA image production smoke" \
  --label "LLaDA image production smoke" \
  --scope code \
...
jq -n '{
  ...
  prompt:"Create a minimal cobalt blue circle centered on a clean white background, flat icon style.",
  size:"1024x1024",
  quality:"low",
  output_format:"jpeg",
  output_compression:20,
  metadata:{client:"llada-image-r760-smoke"}
}' > "$tmp/request.json"
...
jq -er '.data[0].b64_json' "$tmp/response.json" | base64 -d > "$tmp/image.jpg"
test "$(od -An -tx1 -N3 "$tmp/image.jpg" | tr -d ' \n')" = ffd8ff
test "$(tail -c 2 "$tmp/image.jpg" | od -An -tx1 | tr -d ' \n')" = ffd9
...
jq -e '.events | any(
  ...
  and .provider == "llada-image"
  and .upstream_model == "llada-image-turbo-fp8"
  and .status == "ok"
